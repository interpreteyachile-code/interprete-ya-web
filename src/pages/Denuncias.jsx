...
export default function Denuncias() {
  ...
  // ✅ NUEVO: imágenes evidencia (base64)
  ...
  async function onPickImages(e) {
    ...
    for (const f of take) {
      ...
      if (mb > MAX_MB) {
        alert(`La imagen "${f.name}" supera ${MAX_MB}MB. Usa una más liviana.`);
        continue;
      }
  ...
  // ✅ PDF export (sin librerías)
  function printReportAsPDF(r) {
    const w = window.open("", "_blank");
    if (!w) return alert("Bloqueado por el navegador. Permite pop-ups para exportar.");
    ...
    <html>
      ...
      <body>
        ...
        <div>
          ...
          <span class="chip">Fecha: ${escapeHtml(new Date(r.createdAt).toLocaleString("es-CL"))}</span>
        </div>

        <div class="card">
          <div class="row"><span class="label">Título:</span> ${escapeHtml(r.title || "Reporte")}</div>
          <div class="row"><span class="label">Ubicación (texto):</span> ${escapeHtml(r.locationText || "—")}</div>
          <div class="row"><span class="label">Descripción:</span><br/>${escapeHtml(r.description || "—").replaceAll("\n", "<br/>")}</div>
          <div class="row" style="margin-top:10px;"><span class="label">Evidencia (texto):</span><br/>${escapeHtml(r.evidenceText || "—").replaceAll("\n", "<br/>")}</div>
    ...
    <div className="grid gap-4">
      ...
      {!user ? (
        ...
        <div className="tron-card p-6">
          ...
          {/* ✅ NUEVO: imágenes + preview */}
          <div className="mt-3">
            <label className="text-sm text-white/70">📷 Adjuntar fotos (opcional)</label>
            <input
              type="file"
              accept="image/*"
              multiple
            ...
            />
            <div className="text-xs text-white/55 mt-1">
              Máx {MAX_IMAGES} imágenes • hasta {MAX_MB}MB c/u (demo).
            </div>
