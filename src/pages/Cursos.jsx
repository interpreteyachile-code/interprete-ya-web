...
function labelMode(m) {
  return m === "online" ? "💻 Online" : m === "presencial" ? "🏫 Presencial" : "🔁 Mixto";
}

function labelStatus(s) {
  return s === "draft" ? "📝 Borrador" : s === "published" ? "✅ Publicado" : "⏸️ Pausado";
}
...
export default function Cursos() {
  ...
  // perfil docente sordo (según tu proyecto: profileType)
  const isTeacher =
    user?.profileType === "teacher" ||
    user?.profileType === "deaf_teacher" ||
    user?.profileType === "sordo_docente";
  ...
  // Crear curso (solo docente sordo o gerente)
  ...
  const [statusFilter, setStatusFilter] = useState("published"); // default: publicados
  ...
  const visible = useMemo(() => {
    ...
    // si no es gerente ni docente, solo ver publicados
    ...
    if (query) {
      data = data.filter((c) => {
        const blob = `${c.title} ${c.description} ${c.teacherName} ${c.city} ${c.place}`.toLowerCase();
        return blob.includes(query);
      });
    }

    return [...data].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
  }, [allLive, q, canManage, isTeacher, statusFilter, levelFilter, modeFilter]);

  const canCreate = isLogged && (canManage || isTeacher);
  ...
  function submit() {
    if (!canCreate) return alert("Solo docente sordo o gerente puede crear cursos.");
  ...
  // ✅ ACTUALIZADO: enroll = pago demo antes
  function enroll(id) {
    if (!user) return alert("Debes iniciar sesión.");

    // si ya pagó antes, inscribe directo
  ...
  // ✅ NUEVO: confirmar pago + inscribir
  ...
  function del(id) {
    ...
    if (!window.confirm("¿Eliminar curso?")) return; // ✅ FIX ESLint
    deleteCourse(id);
    setTick((x) => x + 1);
    setSelected(null);
  }
  ...
  const isEnrolled = selected && user && selected.enrollments?.some((e) => e.userId === user.id);
  const seatsLeft = selected ? Math.max(0, (selected.seats || 0) - (selected.enrollments?.length || 0)) : 0;
  ...
  return (
    <div className="grid gap-4">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="text-white/70 mt-2">
              Los sordos como docentes: cursos online/presenciales para fortalecer la LSCh y generar ingresos.
            </div>
          ...
          {user && (
            <div className="tron-card p-4">
              ...
              <div className="text-xs text-white/55 mt-1">
                {canManage ? "🧑‍💼 Gerente" : isTeacher ? "🧏‍♂️ Docente Sordo" : "👤 Estudiante"}
              </div>
            </div>
          )}
        </div>
      ...
      {isLogged && myEnrollments.length > 0 && (
        <div className="tron-card p-6">
          ...
          <div className="mt-3 grid md:grid-cols-2 gap-3">
            {myEnrollments.map((c) => (
              <button key={c.id} className="tron-btn text-left" onClick={() => openDetail(c)}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-white/55 mt-1">Docente: {c.teacherName}</div>
                  </div>
                  <Chip>✅ Inscrito</Chip>
                </div>
              </button>
      ...
      {/* CREAR (docente/gerente) */}
      {!canCreate ? null : (
        <div className="tron-card p-6">
          <div className="font-semibold">📝 Crear curso (Docente)</div>
          <div className="text-sm text-white/70 mt-1">
            Crea en borrador → luego Publicar. Cupos y precio en CLP.
          </div>
          ...
          <div className="mt-4 grid md:grid-cols-2 gap-2">
            ...
            <div>
              ...
              <select className="tron-select w-full mt-1" value={level} onChange={(e) => setLevel(e.target.value)}>
            ...
            <div>
              ...
              <select className="tron-select w-full mt-1" value={mode} onChange={(e) => setMode(e.target.value)}>
            ...
            <div>
              ...
              />
              <div className="text-xs text-white/55 mt-1">Vista: {formatCLP(Number(priceCLP) || 0)}</div>
            </div>
      ...
      <div className="tron-card p-6">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="text-xs text-white/60">
            {canManage ? "Vista gerente" : isTeacher ? "Vista docente" : "Vista estudiante"}
          </div>
        </div>
        ...
        <div className="mt-3 grid md:grid-cols-4 gap-2">
          ...
          <select className="tron-select" value={levelFilter} onChange={(e) => setLevelFilter(e.target.value)}>
          ...
          <select className="tron-select" value={modeFilter} onChange={(e) => setModeFilter(e.target.value)}>
          ...
          <div>
            ...
            <div className="mt-4 tron-card p-5">
              ...
              <div className="text-sm text-white/75 mt-3">
                <b>Descripción:</b>
                <div className="text-white/75 mt-1 whitespace-pre-wrap">{selected.description || "—"}</div>
              </div>
            </div>
            ...
            {user && !isMine && selected.status === "published" && (
              <div className="mt-4 tron-card p-5">
                ...
                <div className="text-xs text-white/55 mt-2">
                  (Demo) Ahora pide pago antes de inscribir. Luego conectamos WebPay/MercadoPago.
                </div>
              </div>
            )}

            {/* GESTIÓN DOCENTE / GERENTE */}
            {isMine && (
              <div className="mt-4 tron-card p-5">
                <div className="font-semibold">🧑‍🏫 Gestión del curso</div>

                <div className="mt-3 grid md:grid-cols-3 gap-2">
                  <button className="tron-btn tron-primary font-semibold py-3" onClick={() => setStatus(selected.id, "published")}>
                    ✅ Publicar
                  </button>
                  <button className="tron-btn font-semibold py-3" onClick={() => setStatus(selected.id, "paused")}>
                    ⏸️ Pausar
                  </button>
                  <button className="tron-btn tron-muted font-semibold py-3" onClick={() => setStatus(selected.id, "draft")}>
                    📝 Borrador
                  </button>
                </div>

                <button className="tron-btn w-full mt-3 py-3 font-semibold" onClick={() => del(selected.id)}>
                  🗑️ Eliminar curso
                </button>
                  ...
                  <div className="mt-2 grid md:grid-cols-2 gap-2">
                    {selected.enrollments.map((e) => (
                      <div key={e.userId} className="tron-card p-3">
                        ...
                        <div className="text-xs text-white/55">🕒 {new Date(e.enrolledAt).toLocaleString()}</div>
                      </div>
                    ))}
                  </div>
                )}
        ...
        {!selected ? null : (
          <div>
            ...
            <div className="text-sm text-white/70 mt-2">
              Pago simulado para validar el flujo. Luego conectamos WebPay/MercadoPago.
            </div>
